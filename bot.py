...
conn = sqlite3.connect('submissions_3.db')
cursor = conn.cursor()
...
def update_or_add_problem(submission):
    ...
    cursor.execute('SELECT review_latest FROM completed_problems WHERE title = ?', (title,))
    existing = cursor.fetchone()
    today = datetime.now().strftime('%Y-%m-%d')
    if existing:  #This problem exists -> consider it is a new problem learned today or a revise problem
        review_latest = existing[0]
        print(f"{title} existed")
        cursor.execute('SELECT accepted_date FROM completed_problems WHERE title = ?', (title,))
        accepted_date_result = cursor.fetchone()[0]
        if time_stamp < accepted_date_result: #Submissions from latest to oldest -> overlap -> update accepted_date
            cursor.execute('UPDATE completed_problems SET accepted_date = ? WHERE title = ?', (time_stamp, title))
        elif review_latest != today:
            ...
            if review_times_result:
                ...
                if review_next_result:
                    # Calculate new `review_next` date based on `review_latest`
                    review_next = (datetime.strptime(review_latest, '%Y-%m-%d') + timedelta(days=2 * (review_times + 1))).strftime('%Y-%m-%d')
            ...
            print("This Submissions have just accepted today for the first time, do nothing")
        ...
        cursor.execute('INSERT INTO completed_problems (title, accepted_date, review_next, review_latest, review_times) VALUES (?, ?, ?, ?, ?)', 
                       (title, accepted_date, review_next, review_latest, 0))
...
class MyClient(discord.Client):
    ...
    async def my_background_task(self):
        ...
        cursor.execute('SELECT title, accepted_date, review_next , review_latest FROM completed_problems')
        all_problems = cursor.fetchall()

        for title, accepted_date, review_next, review_latest in all_problems:
            if accepted_date == today:
                new_problems.append(f"{title}: {get_problem_link(convert_to_slug(title))}")
            if review_next <= today and review_latest != today:
                to_review_problems.append(f"{title}: {get_problem_link(convert_to_slug(title))}")
            if review_latest == today and accepted_date != today:
